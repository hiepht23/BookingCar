...
public class LinearListView extends IcsLinearLayout {
	...
	private static int[] rStyleableLinearListView = new int[] {
	/* 0 */android.R.attr.entries,
	/* 1 */R.attr.dividerThickness };

	private static int linearListViewEntries = 0;
	private static int linearListViewDividerThickness = 1;
	...
	public LinearListView(Context context, AttributeSet attrs) {
		...
		TypedArray array = context.obtainStyledAttributes(attrs,
				rStyleableLinearListView);

		// Use the thickness specified, zero being the default
		final int thickness = array.getDimensionPixelSize(
				linearListViewDividerThickness, 0);
		if (thickness != 0) {
			setDividerThickness(thickness);
		}

		CharSequence[] entries = array.getTextArray(linearListViewEntries);
		if (entries != null) {
	...
	public interface OnItemClickListener {
		 ...
		 */
		void onItemClick(LinearListView parent, View view, int position,
                         long mId);
	}
	 ...
	 */
	public boolean performItemClick(View view, int position, long mId) {
		if (mOnItemClickListener != null) {
			playSoundEffect(SoundEffectConstants.CLICK);
			mOnItemClickListener.onItemClick(this, view, position, mId);
			return true;
		}
